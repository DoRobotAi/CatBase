...
[English](README.md) · [简体中文](#) · [官方文档](doc/catbase.md) · [官网](http://catbase-lang.com) · [公司官网](http://dorobot.net)
...
CatBase 采用**自定义最终用户许可协议（EULA）** 分发，**不是开源许可证**。完整条款请参阅 [LICENSE](LICENSE) 文件。

**许可摘要**：

| ✅ 您**可以** | ❌ 您**不可以** |
|------|------|
| 下载并安装本软件 | 对本软件进行逆向工程、反编译或反汇编 |
| 使用本软件编译 CatBase 源代码 | 修改、补丁或创建本软件本身的衍生作品 |
| 分发**您使用本软件编译的**程序 / 库 | 在不同许可下重新分发本软件的二进制文件 |
|  | 删除或更改任何版权、署名或许可声明 |

> ⚠️ **编译产物归您所有** — 您使用 CatBase 编译的程序和库的所有权归您，不受本 EULA 约束。
